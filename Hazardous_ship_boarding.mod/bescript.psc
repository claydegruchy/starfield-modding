...
Group BoardingProperties collapsedonbase
  ...
  Float Property MaxSimultaneousBoardersPercent = {{BoardingProperties.MaxSimultaneousBoardersPercent}} Auto Const
  ...
  Int Property MinBoardingWaveSize = {{BoardingProperties.MinBoardingWaveSize}} Auto Const
  { DEFAULT=2. The minimum wave size for a wave of enemies boarding the player's ship. }
  Int Property MaxBoardingWaveSize = {{BoardingProperties.MaxBoardingWaveSize}} Auto Const
...
Group HeatLeachProperties collapsedonbase
  Float Property HeatLeechChance = {{HeatLeachProperties.HeatLeechChance}} Auto Const
...
Event OnQuestStarted()
  ...
  ; ====== register mod controls =====
  Bool allowCustomHazard = True
  Bool allowBoarders = True

  ; dont allow if isDropshipEncounter is true
  If isDropshipEncounter
    allowCustomHazard = False 
    allowBoarders = False
  EndIf
  ; dont allow if isSurfaceEncounter is true
  If isSurfaceEncounter
    allowCustomHazard = False 
    allowBoarders = False
  EndIf
  ...
  If !isDropshipEncounter
    ...
    ; here we set the hazard to a random one from the list
  ...
  If isSurfaceEncounter
    ...
    EndIf
  EndIf
  If ShouldBoardPlayersShip && allowBoarders && Utility.RandomFloat(0.0, 1.0) < BoardingChance
    
    playerShipRef = PlayerShip.GetRef() as spaceshipreference
    playerShipDockingDoorRef = PlayerShipLoadDoor.GetRef()
...
Event Actor.OnLocationChange(Actor akSource, Location akOldLoc, Location akNewLoc)
  If akSource == Game.GetPlayer()
    ...
    ElseIf akOldLoc == enemyShipInteriorLoc
      ...
      EndIf
Else
      WorldSpace enemyWorldSpace = enemyShipRef.GetWorldSpace()
      WorldSpace playerWorldSpace = playerShipRef.GetWorldSpace()
      If enemyWorldSpace != playerWorldSpace
        Self.CleanupAndStop()
      ElseIf !playerWorldSpace ;We're in space
        Cell enemyCell = enemyShipRef.GetParentCell()
        Cell playerCell = playerShipRef.GetParentCell()
